...
export class FilterUnitsService {
  ...
  filter(results: Location[], showClosed: boolean, hour: string){
    ...
    if (hour) {
      const OPEN_HOUR = OPENING_HOUR[hour as HOUR_INDEX].first
      const LAST_OPEN_HOUR = OPENING_HOUR[hour as HOUR_INDEX].last
      return intermediateResults.filter(location => this.filterUnits(location, OPEN_HOUR, LAST_OPEN_HOUR));
    } else {
